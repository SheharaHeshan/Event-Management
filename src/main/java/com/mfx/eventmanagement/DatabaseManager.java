...
public class DatabaseManager {
    ...
    public int registerInitialUser(String firstName, String lastName, String email, String passwordHash) {
        // NOTE: is_verified is set to 1, assuming email verification passed just before this step.
        ...
        try (Connection conn = getConnection();
            ...
            // Handle duplicate email (if email column is unique) or other DB errors
            System.err.println("Error registering initial user for email " + email + ": " + e.getMessage());
            e.printStackTrace();
        }
        return userId;
    }
    ...
    /**
     * Updates the user's profile information after initial registration.
     * It is assumed the user already exists in the 'users' table by their email.
     *
     * @param email The user's email, used to identify the user.
     * @param age User's age.
     * @param gender User's gender.
     * @param phoneNumber User's phone number.
     * @param address User's address.
     * @param profilePath Path to the user's profile picture.
     * @return true if the profile was successfully updated, false otherwise.
     */
    public boolean completeUserProfile(String email, int age, String gender, String phoneNumber, String address, String profilePath) {
        String sql = "UPDATE users SET age = ?, gender = ?, phone_number = ?, address = ?, profile_picture_path = ?, is_profile_complete = 1 WHERE email = ?";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            // WHERE clause: use email to identify the user
